import type { Translations } from "./fa";

const en: Translations = {
  common: {
    next: "Next",
    previous: "Previous",
    page: "Page",
    loading: "Loading...",
    save: "Save",
    cancel: "Cancel",
    delete: "Delete",
    edit: "Edit",
    confirm: "Confirm",
    close: "Close",
    search: "Search...",
    actions: "Actions",
    status: "Status",
    all: "All",
    yes: "Yes",
    no: "No",
    id: "ID",
    name: "Name",
    date: "Date",
    amount: "Amount",
    description: "Description",
    error: "Error",
    success: "Success",
    noData: "No data found",
    submit: "Submit",
    back: "Back",
    refresh: "Refresh",
    toman: "Toman",
    Total: "Total",
    active: "Active",
    inactive: "Inactive",
    copy: "Copy",
    add: "Add",
    saving: "Saving...",
  },

  auth: {
    loginTitle: "Admin Panel",
    loginSubtitle: "Bot Management Panel",
    loginText: "",
    loginButton: "Login with Telegram",
    loggingIn: "Logging in...",
    logout: "Logout",
    notAuthorized: "You are not authorized as admin",
    telegramIdPlaceholder: "Telegram ID",
    passwordPlaceholder: "Password",
  },

  nav: {
    dashboard: "Dashboard",
    products: "Products",
    orders: "Orders",
    users: "Users",
    tickets: "Tickets",
    wallet: "Wallet",
    discounts: "Discounts",
    referrals: "Referrals",
    perks: "Perks",
    schedules: "Schedules",
    "session-chats": "Session Chats",
    broadcast: "Broadcast",
    settings: "Settings",
    pages: "Pages",
    account: "My Account",
  },

  dashboard: {
    title: "Dashboard",
    todayOrders: "Today's Orders",
    todayRevenue: "Today's Revenue",
    newUsersToday: "New Users Today",
    openTickets: "Open Tickets",
    pendingAdmin: "Pending Admin",
    totalUsers: "Total Users",
    todaySchedules: "Today's Sessions",
    activeSchedules: "Active Sessions Now",
    needsAttention: "Needs Attention",
    last7Days: "Last 7 Days Sales",
  },

  products: {
    title: "Products",
    newProduct: "+ New Product",
    orderChangesNotice: "Orders changed",
    displayOrder: " Product order number",
    searchPlaceholder: "Search...",
    saveOrder: "Save Order",
    name: "Name",
    category: "Category",
    deliveryType: "Delivery Type",
    stock: "Stock",
    active: "Active",
    inactive: "Inactive",
    toggle: "Toggle Status",
    updateStock: "Update Stock",
    minStock: "Min Stock",
    namePlaceholder: "e.g. Netflix Premium 1 Month",
    requiresEmail: "Requires Email",
    requiresOtp: "Requires OTP",
    requiresLogin: "Requires Login",
    requiresRegion: "Requires Region",
    plans: "Plans",
    isRenewable: "Renewable",
    warrantyDays: "Warranty Days",
    warrantyDaysHint:
      "Days admin is responsible for the item (0 = no warranty)",
    terms: "Terms & Conditions",
    termsPlaceholder: "Usage terms for this product...",
    maxPerUser: "Max per User",
    maxPerUserHint: "0 = no limit",
    customEmojiHint:
      "آیدی عددی Custom Emoji — در دکمه تلگرام نمایش داده می‌شود",
    planRequirementsHint:
      "Per-plan requirements (email, OTP, login) are set in the Plans section below ↓",
    regions: "Regions",
    regionsHint:
      "Available regions for user to choose in the bot (e.g. Egypt, Nigeria)",
    addRegion: "+ Add Region",
    noRegions: "No regions — user enters free text",
    deliveryTypes: {
      automatic: "Automatic",
      manual: "Manual",
      custom_schedule: "Custom Schedule",
      invite: "Invite",
      code: "Code",
      family_join: "Family Join",
      renewable: "Renewable",
      reservation: "Reservation",
    },
    planModal: {
      newPlan: "New plan +",
      plansTitle: "Plans",
      deletePlanConfirm: "Delete this plan?",
      cost: "Cost",
      time: "Time",
      timeType: "Time type",
      day: "Day",
      mounth: "Mounth",
      year: "Year",
      tartib: "Order",
      requirements: "Plan Requirements",
      requiresEmail: "Requires Email",
      requiresOtp: "Requires OTP",
      requiresLogin: "Requires Live Login",
      requiresRegion: "Requires Region",
      regions: "regions",
      addRegion: "add region",
      placeholder: {
        regionName: "Region like(Egypt)",
        price: "price",
      },
    },
    configModal: {
      title: "Configs",
      addConfig: "Add Config",
      bulkAdd: "Bulk Add",
      configData: "Config Data (link/key)",
      configDataPlaceholder: "e.g. VPN link or activation key...",
      label: "Label (optional)",
      labelPlaceholder: "e.g. Server US-1",
      bulkPlaceholder: "One config per line...",
      bulkHelp: "Each line = one config. Empty lines are ignored.",
      available: "Available",
      used: "Used",
      deleteConfirm: "Delete this config?",
      manageConfigs: "Configs",
      bulkSuccess: "{count} configs added",
    },
    inventory: {
      manage: "Inventory",
      title: "Inventory Management",
      available: "Available",
      reserved: "Reserved",
      used: "Used",
      dead: "Dead",
      addItem: "Add Item",
      bulkAdd: "Bulk Add",
      emailLabel: "Email",
      passwordLabel: "Password",
      extraDataLabel: "Extra Data",
      bulkPlaceholder: "Each line: email:password or email:password:extra",
      bulkHelp: "One item per line",
      bulkSuccess: "{count} items added",
      deleteConfirm: "Delete this item?",
      markDead: "Mark as Dead",
      markDeadConfirm: "Mark as dead?",
      deadReasonPlaceholder: "Reason...",
      orderId: "Order ID",
      stats:
        "{available} available · {reserved} reserved · {used} used · {dead} dead",
    },
    deliveryItems: {
      manage: "Delivery Items",
      title: "Delivery Items",
      addItems: "Add Item",
      contentPlaceholder:
        "One item per line\nemail:password\nor a link\nor any text",
      bulkSuccess: "{count} items added",
    },
    categories: {
      title: "Categories",
      new: "+ New Category",
      namePlaceholder: "e.g. Video Subscriptions",
      customEmojiId: "Custom Emoji ID",
      enable: "Enable",
      disable: "Disable",
    },
    gifts: {
      title: "Purchase Gifts",
      subtitle:
        "After each order is delivered, one of these items is sent to the user as a one-time gift. If the list is empty, nothing is sent.",
      new: "+ Add Gift",
      content: "Gift content",
      contentPlaceholder: "Gift account info (can be multi-line)...",
      statusCol: "Status",
      available: "Available",
      used: "Sent",
      delete: "Delete",
      deleteConfirm: "Delete this gift item?",
      empty: "No gift items yet",
      template: {
        title: "Gift Message Template",
        hint: "Use {item} for the gift content and {orderId} for the order number.",
        enabled: "Gift sending enabled",
        fa: "Persian template",
        en: "English template",
        ru: "Russian template",
        save: "Save Template",
        saved: "Template saved",
      },
    },
    tabs: {
      all: "All products",
    },
  },

  orders: {
    title: "Orders",
    pendingAdmin: "Pending Admin",
    scheduled: "Scheduled",
    waitingInvite: "Waiting Invite",
    all: "All",
    pendingPayment: "Verify Payment",
    approvePayment: "Approve Payment",
    rejectPayment: "Reject Payment",
    rejectReason: "Rejection reason (optional)",
    noPendingPayments: "No orders awaiting payment verification",
    collectedInfo: "Customer Info",
    paymentMethods: {
      card: "Bank Card",
      crypto: "Crypto (USDT)",
      zarinpal: "ZarinPal",
      wallet: "Wallet",
    },
    userId: "User",
    product: "Product",
    total: "Total",
    payment: "Payment",
    paymentWay: "Payment way",
    date: "Date",
    actions: "Actions",
    updateStatus: "Update Status",
    refund: "Refund",
    refundReason: "Refund Reason",
    state: "state",
    statuses: {
      pending_payment: "Pending Payment",
      paid: "Paid",
      pending_admin: "Pending Admin",
      waiting_schedule: "Waiting Schedule",
      scheduled: "Scheduled",
      in_progress: "In Progress",
      completed: "Completed",
      cancelled: "Cancelled",
      refunded: "Refunded",
      waiting_invite: "Waiting Invite",
      invite_sent: "Invite Sent",
      rescheduled: "Rescheduled",
    },
  },

  users: {
    title: "Users",
    searchPlaceholder: "Search username / name...",
    username: "Username",
    role: "Role",
    walletBalance: "Wallet Balance",
    blocked: "Blocked",
    active: "Active",
    profile: "Profile",
    block: "Block",
    unblock: "Unblock",
    blockReason: "Block Reason",
    adjustWallet: "Adjust Wallet",
    chargeWallet: "Charge Wallet",
    deductWallet: "Deduct Wallet",
    walletAmount: "Amount (Toman)",
    walletDescDefault: "Manual adjustment by admin",
    changeRole: "Change Role",
    registeredAt: "Registered At",
    orderCount: "Total Orders",
    referralCount: "Referred Users",
    referralLink: "Referral Link",
    referralCode: "Referral Code",
    copied: "Copied!",
    roles: {
      customer: "Customer",
      support: "Support",
      admin: "Admin",
      super_admin: "Super Admin",
    },
  },

  tickets: {
    title: "Tickets",
    subject: "Subject",
    open: "Open",
    closed: "Closed",
    inProgress: "In Progress",
    waitingUser: "Waiting for User",
    resolved: "Resolved",
    reply: "Reply",
    close: "Close Ticket",
    assign: "Assign",
    viewDetails: "View",
    newMessages: "New Messages",
    messagePlaceholder: "Reply message...",
    allStatuses: "All Statuses",
    allPriorities: "All Priorities",
    filterAllType: "All — Type",
    priority: {
      urgent: "Urgent",
      high: "High",
      normal: "Normal",
      low: "Low",
    },
    type: {
      support: "Support",
      order: "Order Issue",
      report: "Report",
    },
    waitingSupport: "Waiting for Support",
  },

  wallet: {
    title: "Wallet",
    deposit: "Deposit",
    withdraw: "Withdraw",
    type: "Type",
    balance: "Balance",
    transactions: "Transactions",
    credit: "Credit",
    debit: "Debit",
    allTypes: "All Types",
    allSources: "All Sources",
    totalCredit: "Total Deposits",
    totalPurchase: "Total User Purchases",
    totalBalance: "Total Wallet Balance",
    txCount: "transactions",
    orderCount: "orders",
    bySourceTitle: "Summary by Source",
    clearFilter: "Clear Filter",
    searchPlaceholder: "Search name / username...",
    prevPage: "Previous",
    nextPage: "Next",
    page: "Page",
    sources: {
      purchase: "Purchase",
      recharge: "Wallet Recharge",
      refund: "Refund",
      referral: "Referral Reward",
      perk: "Perk Reward",
      admin_adjustment: "Admin Adjustment",
    },
  },

  discounts: {
    title: "Discount Codes",
    newDiscount: "+ New Discount",
    newCode: "+ New Code",
    code: "Discount Code",
    percentage: "Percentage",
    maxUses: "Max Uses",
    usedCount: "Used Count",
    expiresAt: "Expires At",
    newCodeTitle: "New Discount Code",
    editTitle: "Edit Discount Code",
    filterAllType: "All — Type",
    typePercentage: "Percentage",
    typeFixed: "Fixed",
    filterAllStatus: "All — Status",
    filterAllExpiry: "All — Expiry",
    expired: "Expired",
    cap: "Cap",
    uses: "Uses",
    minLabel: "Min",
    expiresLabel: "Expires",
    confirmDelete: "Delete this?",
    codeLabel: "Discount Code",
    typeLabel: "Type",
    descriptionLabel: "Description",
    descriptionPlaceholder: "Optional description",
    valuePercent: "Value (%)",
    valueFixed: "Value (Toman)",
    maxDiscountLabel: "Max Discount (Toman)",
    optional: "Optional",
    maxUsesTotal: "Max Total Uses",
    unlimited: "Unlimited",
    maxUsesPerUserLabel: "Max Uses Per User",
    minOrderLabel: "Min Order Amount",
    isActiveLabel: "Active",
    saving: "Saving...",
  },

  referrals: {
    title: "Referral Rewards",
    referrer: "Referrer",
    referred: "Referred",
    reward: "Reward",
    earnedAt: "Earned At",
    totalAwarded: "Total Awarded",
    cases: "cases",
    pendingReview: "Pending Review",
    topReferrer: "Top Referrer",
    referralsCount: "referrals",
    allStatuses: "All Statuses",
    referredUser: "Referred User",
    award: "Award",
    cancelReward: "Cancel",
    prevPage: "Previous",
    nextPage: "Next",
    page: "Page",
    statuses: {
      pending: "Pending",
      awarded: "Awarded",
      cancelled: "Cancelled",
    },
    rewardTypes: {
      wallet_credit: "Wallet Credit",
      discount: "Discount Code",
    },
  },

  perks: {
    title: "Perks",
    tasks: "Tasks",
    requests: "Requests",
    newTask: "+ New Task",
    editTask: "Edit Task",
    createTask: "New Task",
    taskTitleLabel: "Title",
    taskTitlePlaceholder: "Task title",
    taskDescLabel: "Description",
    taskDescPlaceholder: "Optional description",
    taskTypeLabel: "Task Type",
    rewardTypeLabel: "Reward Type",
    rewardValueLabel: "Reward Value (Toman)",
    rewardValuePlaceholder: "e.g. 50000",
    maxRewardsLabel: "Max Rewards",
    maxRewardsHint: "Empty = unlimited",
    maxRewardsPlaceholder: "Leave empty",
    expiresAtLabel: "Expires At",
    optionalHint: "optional",
    allStatuses: "All Statuses",
    noTasks: "No tasks defined yet",
    noRequests: "No requests found",
    deleteTaskConfirm: "Delete this task?",
    saving: "Saving...",
    colTitle: "Title",
    colType: "Type",
    colReward: "Reward",
    colValue: "Value",
    colProgress: "Progress",
    colExpires: "Expires",
    colUser: "User",
    colTask: "Task",
    colRewardType: "Reward Type",
    colRewardValue: "Reward Value",
    verify: "Verify",
    reject: "Reject",
    taskTypes: {
      join_channel: "Join Channel",
      invite_friend: "Invite Friend",
      instagram_story: "Instagram Story",
      tweet: "Tweet",
      review: "Write Review",
      first_purchase: "First Purchase",
      renew_subscription: "Renew Subscription",
      complete_profile: "Complete Profile",
    },
    rewardTypes: {
      wallet_credit: "Wallet Credit",
      discount: "Discount Code",
      free_product: "Free Product",
    },
    requestStatuses: {
      pending: "Pending",
      verified: "Verified",
      completed: "Rejected",
      claimed: "Claimed",
    },
    isActiveLabel: "Active",
    prevPage: "Prev",
    nextPage: "Next",
    page: "Page",
    progressLabel: "Progress",
    expiresLabel: "Expires",
  },

  sessionChats: {
    title: "Session Chats",
    open: "Open",
    closed: "Closed",
    all: "All",
    allStatuses: "All Statuses",
    sendMessage: "Send Message",
    closeChat: "Close Chat",
    closeChatConfirm: "Are you sure? This chat will be closed.",
    messagePlaceholder: "Message...",
    noChats: "No chats found",
    colUser: "User",
    colProduct: "Product",
    colSchedule: "Session",
    colStatus: "Status",
    colDate: "Date",
    viewChat: "View Chat",
    admin: "Admin",
    user: "User",
    noMessages: "No messages yet",
  },

  schedules: {
    title: "Order Schedules",
    slots: "slots",
    noSlots: "No slots for this day",
    complete: "Complete",
    reminder: "Reminder",
    reminderSent: "Reminder Sent",
    colTimeSlot: "Time Slot",
    colProduct: "Product",
    colUser: "User",
    colOrder: "Order",
    colCapacity: "Capacity",
    statusLabels: {
      available: "Available",
      full: "Full",
      in_progress: "In Progress",
      completed: "Delivered",
    },
  },

  broadcast: {
    title: "Broadcast Message",
    recipients: "Select Recipients",
    sendTo: "Send to:",
    allUsers: "All Users",
    productBuyers: "Product Buyers",
    roleFilter: "Specific Role",
    languageFilter: "User Language",
    expiringSubscriptions: "Expiring Subscriptions",
    selectProduct: "Select product...",
    selectRole: "Select role...",
    selectLanguage: "Select language...",
    productLabel: "Product:",
    roleLabel: "Role:",
    languageLabel: "Language:",
    languageFa: "Persian",
    languageEn: "English",
    languageRu: "Russian",
    expiresInLabel: "Expires in:",
    tomorrow: "Tomorrow",
    in3Days: "3 days",
    in7Days: "7 days",
    messageSection: "Message",
    messageFormat: "Message format:",
    messageLabel: "Message text:",
    characters: "characters",
    sendSection: "Send",
    previewBtn: "Preview Recipients Count",
    calculating: "Calculating...",
    recipientsCount: "Recipients:",
    people: "people",
    sendBtn: "Send Message",
    sendBtnTo: "Send Message to {{count}} people",
    sending: "Sending...",
    takePreviewFirst: "Take preview first",
    sentTitle: "Message Sent ✓",
    totalLabel: "Total",
    successLabel: "Success",
    failedLabel: "Failed",
    invalidFilter: "Invalid filter",
    confirmSend: "Are you sure? Message will be sent to {{count}} people.",
    placeholder:
      "Hello FIRST_NAME\n**Important update**\n__underlined text__\n||spoiler text||\n''copyable code''\n{quoted text}\n[1222564456546]",
    syntaxGuideTitle: "Auto-format guide",
    autoFormatHint:
      "Your message is converted to HTML automatically and sent with parse_mode=HTML.",
    syntaxVariables: "Variables: FIRST_NAME, LAST_NAME, USERNAME, USER_ID",
    syntaxBold: "Bold: **text**",
    syntaxUnderline: "Underline: __text__",
    syntaxSpoiler: "Spoiler: ||text||",
    syntaxCode: "Copy/code style: ''text''",
    syntaxQuote: "Quote: {text}",
    syntaxEmoji: "Custom emoji: [1222564456546]👑",
    variablesHint:
      "Hint: use FIRST_NAME, LAST_NAME, USERNAME, USER_ID to inject user data.",
    htmlPlaceholder: "Hello <b>{first_name}</b>\nYour message...",
    markdownPlaceholder: "Hello *{first_name}*\nYour message...",
  },

  settings: {
    title: "Settings",
    adminsTab: "Admins",
    logsTab: "System Logs",
    addAdmin: "+ Add Admin",
    addAdminTitle: "Add New Admin",
    editAdminTitle: "Edit Admin",
    telegramId: "Telegram User ID",
    telegramIdPlaceholder: "e.g. 123456789",
    displayName: "Display Name",
    displayNamePlaceholder: "Display name in panel",
    roleLabel: "Role",
    allowedSections: "Allowed Sections",
    allSectionsHint: "Empty = all",
    notes: "Internal Notes",
    notesPlaceholder: "Optional notes about this admin...",
    colName: "Name",
    colTelegramUser: "Telegram User",
    colRole: "Role",
    colAccess: "Access",
    colLastLogin: "Last Login",
    colLoginCount: "Logins",
    allSections: "All",
    noAdmins: "No admins found",
    allSeverities: "All Severities",
    allEntities: "All Entities",
    colAdmin: "Admin",
    colAction: "Action",
    colEntity: "Entity",
    colDesc: "Description",
    colSeverity: "Severity",
    noLogs: "No logs found",
    prev: "Previous",
    next: "Next",
    page: "Page",
    deactivate: "Deactivate",
    activate: "Activate",
    deleteAdminConfirm: "Delete {{name}}?",
    roles: {
      super_admin: "Owner",
      admin: "Admin",
      manager: "Manager",
      support: "Support",
      operator: "Operator",
    },
    sectionLabels: {
      products: "Products",
      orders: "Orders",
      tickets: "Tickets",
      users: "Users",
      wallet: "Wallet",
      discounts: "Discounts",
      referrals: "Referrals",
      perks: "Perks",
      schedules: "Schedules",
      broadcast: "Broadcast",
      settings: "Settings",
    },
    forceJoinTab: "Force Join",
    paymentTab: "Payment",
    backupTab: "Backup",
    comingSoon: "Coming soon...",
    forceJoin: {
      addTitle: "Add Force Join Channel",
      editTitle: "Edit Channel",
      description: "Users must join these channels before using the bot.",
      addChannel: "+ Add Channel",
      channelId: "Channel ID",
      channelIdPlaceholder: "e.g. @MyChannel or -1001234567890",
      channelUrl: "Invite Link / URL",
      channelUrlPlaceholder: "e.g. https://t.me/MyChannel",
      channelName: "Display Name",
      channelNamePlaceholder: "Channel name shown to users",
      order: "Display Order",
      orderLabel: "Order",
      noChannels: "No channels defined",
      deleteConfirm: "Delete «{{name}}»?",
    },
    payment: {
      cardsTitle: "Bank Card Numbers",
      cardsDesc: "Bank cards displayed to the user during card payment",
      addCard: "+ Add Card",
      cardNumber: "Card Number",
      cardNumberPlaceholder: "e.g. 6037-9975-1234-5678",
      holderName: "Card Holder Name",
      holderNamePlaceholder: "Full name",
      bankName: "Bank Name (optional)",
      bankNamePlaceholder: "e.g. Bank Melli",
      addCardTitle: "Add Bank Card",
      editCardTitle: "Edit Bank Card",
      deleteCardConfirm: "Delete card «{{name}}»?",
      noCards: "No cards defined",
      zarinpalTitle: "Zarinpal Gateway",
      zarinpalDesc: "Connection settings for Zarinpal payment gateway",
      zarinpalEnabled: "Enable Zarinpal",
      merchantId: "Merchant ID",
      merchantIdPlaceholder: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
      callbackUrl: "Public Callback URL",
      callbackUrlPlaceholder:
        "https://example.com/api/public/payments/zarinpal/wallet/callback",
      callbackUrlHint:
        "Register this URL in ZarinPal so the bot can auto-check and finalize wallet payments",
      sandboxMode: "Sandbox Mode",
      cryptoTitle: "Crypto Payment (USDT)",
      cryptoDesc: "Settings for crypto wallet payment",
      cryptoEnabled: "Enable Crypto",
      cryptoAddress: "Wallet Address",
      cryptoAddressPlaceholder: "USDT wallet address",
      cryptoNetwork: "Network",
      exchangeRate: "Exchange Rate (Toman per USDT)",
      exchangeRatePlaceholder: "e.g. 55000",
      exchangeRateHint: "How many Tomans per 1 USDT",
      saveConfig: "Save Settings",
      configSaved: "Settings saved",
      cardEnabledLabel: "Card payment enabled",
    },
    backup: {
      title: "Backup Settings",
      desc: "Scheduled database backup and destination channel",
      enabled: "Enable automatic backup",
      channelId: "Telegram Channel ID",
      channelIdPlaceholder: "-1001234567890",
      channelIdHint: "Backup file will be sent to this channel",
      cronSchedule: "Schedule (Cron)",
      cronSchedulePlaceholder: "0 3 * * *",
      cronHelp: "Example: 0 3 * * * = every day at 3:00 AM",
      lastBackup: "Last Backup",
      lastBackupStatus: "Last Backup Status",
      lastBackupSize: "Last Backup Size",
      runNow: "Backup Now",
      running: "Running backup...",
      saveConfig: "Save Settings",
      configSaved: "Settings saved",
      noBackupYet: "No backup has been taken yet",
      statusSuccess: "Success",
      statusFailed: "Failed",
      backupSuccess: "Backup sent successfully",
      backupFailed: "Backup failed",
    },
    botControl: {
      title: "Bot Control",
      desc: "Enable/disable main bot features",
      maintenanceMode: "Maintenance Mode (Bot Off)",
      maintenanceModeSub: "Regular users cannot use the bot in this mode",
      maintenanceMessage: "Custom Maintenance Message",
      maintenanceMessagePlaceholder: "Message shown to users...",
      referralEnabled: "Referral System",
      referralEnabledSub: "Invite friends program",
      shopEnabled: "Shop",
      shopEnabledSub: "Product purchase section",
      saveConfig: "Save Settings",
      configSaved: "Bot control settings saved",
    },
  },

  account: {
    title: "My Account",
    panelInfo: "Admin Panel Info",
    botInfo: "Bot User Info",
    notifications: "Notification Settings",
    displayName: "Display Name",
    adminId: "Admin ID",
    telegramId: "Telegram Numeric ID",
    username: "Telegram Username",
    role: "Panel Role",
    lastLogin: "Last Login",
    loginCount: "Login Count",
    accessSections: "Access Sections",
    allSections: "All Sections",
    firstName: "First Name",
    lastName: "Last Name",
    walletBalance: "Wallet Balance",
    botRole: "Bot Role",
    referralCode: "Referral Code",
    language: "Language",
    registeredAt: "Registered in Bot",
    notifyOrders: "Order Notifications",
    notifyWallet: "Wallet Notifications",
    notifyPromotions: "Promotion Notifications",
    notifyReferrals: "Referral Notifications",
    notifyStock: "Stock Notifications",
    changePassword: "Change Password",
    currentPassword: "Current Password",
    newPassword: "New Password",
    confirmPassword: "Confirm New Password",
    passwordChanged: "Password changed successfully",
    passwordMismatch: "New password and confirmation do not match",
    langFa: "FA",
    langEn: "EN",
    langRu: "RU",
  },
};

export default en;
